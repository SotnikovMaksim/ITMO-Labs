...
public class BinarySearchMissing {
    ...
    public static void main(String[] args) {
        ...
        // :NOTE: если условие повторяется несколько раз, его можно записать в "переменную" и использовать везде её
...
//        int R = iteratedBinarySearch(x, a);
    ...
    public static int iteratedBinarySearch(int x, int[] a) {
        ...
        while (right - left > 1) {
            ...
            // middle <= right - 1
            if (a[middle] < x) {
                // :NOTE: опечатка, не тот знак в a[middle] > x
                // (right - left > 1) && (left < right) && (a[middle] > x)
                // :NOTE: стоило сказать чуть более доказательно, почему смещаем границу
                //        например: a[left] < a[middle] < x < a[right]
                left = middle;
            } else {
                // (right - left > 1) && (left < right) && (a[middle] <= x)
                right = middle;
